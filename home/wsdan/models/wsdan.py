...
"""
'''
修改记录：fpn,gap,权重,filename = wsdan_fpn_gap_w
'''
import os
import logging
import numpy as np
...
visualize = True
if visualize:
    os.makedirs('./FGVC/VehicleID/atten_visualize/', exist_ok=True)
...
class MyLayer(torch.nn.Module):
    def __init__(self,M):
        ...
        self.M = M
        self.par_list = torch.nn.Parameter(torch.randn((self.M))*np.sqrt(1/self.M)) 
...
class BAP(nn.Module):#GAP全局平均池化，GMP全局最大池化
    ...
    def forward(self, features, attentions):
        ...
        if self.pool is None:
            b = torch.einsum('imjk,injk->imn', (attentions, features)) / float(H * W)
            feature_matrix = []
            for i in range(M):
                AiF = b[:,i,:]
