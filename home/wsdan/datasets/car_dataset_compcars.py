...
# DATAPATH = '/home/FGVC/StanfordCars'
DATAPATH = '/home/CompCars/data'
...
class CarDataset(Dataset):
    ...
    def __init__(self, phase='train', resize=500):
        ...
        self.phase = phase
        self.resize = resize
        self.num_classes = 1993

        if phase == 'train':
            list_path = os.path.join(DATAPATH, 'devkit', 'compcars_train.mat')
            ## self.image_path = os.path.join(DATAPATH, 'cars_train')
        else:
            list_path = os.path.join(DATAPATH,'devkit','compcars_test.mat')
        self.image_path = os.path.join(DATAPATH, 'image')

        list_mat = loadmat(list_path)
        self.images = [f.item() for f in list_mat['annotations']['fname'][0]]
        self.labels = [f.item() for f in list_mat['annotations']['model'][0]]
        # transform
